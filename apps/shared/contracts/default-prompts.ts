/** Built-in agent prompt templates, moved verbatim from the worker prompt
 *  registry (apps/worker/src/lib/prompts.ts). Kept byte-identical to that
 *  source so a later stage can point the worker runtime at these constants
 *  without changing agent behavior. Do not reflow or edit the bodies. */
