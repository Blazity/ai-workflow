...
export function SendMessageFields(props: BlockRendererProps) {
  ...
  return (
    <>
      ...
      <ConfigNote>
        {sendOn === "always"
          ? node.v2
            ? "Posts your message on its own in this ticket's conversation whenever this block runs. Use the Value picker to add a PR link when one is available."
            : "Posts your message on its own in this ticket's conversation whenever this block runs. Add {{pr_url}} if you want a PR link."
          : "Adds your message to the pull request notification, only after a pull request is published."}
      </ConfigNote>
    </>
  );
}
