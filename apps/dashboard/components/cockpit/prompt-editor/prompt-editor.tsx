...
function ContextMenu({
  ...
  const { mounted, state } = useEnterExit(at !== null, MOTION_BASE_MS);
...
export function PromptEditor({
  ...
  const varAnchorRef = useRef<HTMLSpanElement>(null);
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap shrink-0 items-center gap-y-1 gap-x-0.5 border-b border-neutral-200 px-1.5 py-1">
        ...
        {!raw && (
          <span ref={varAnchorRef} className="ml-0.5 inline-flex">
            <Button
              variant="ghost"
              size="sm"
              type="button"
