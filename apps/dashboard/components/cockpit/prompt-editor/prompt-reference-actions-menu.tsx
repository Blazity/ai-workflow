...
export function PromptReferenceActionsMenu({
  ...
  const itemClass =
    "flex min-h-10 w-full cursor-pointer items-center rounded-[3px] px-3 text-left font-mono text-[10px] text-neutral-700 transition-colors duration-[var(--motion-fast)] hover:bg-off-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-mariner-200 disabled:cursor-default disabled:opacity-50";
  return createPortal(
    <div
      ref={menuRef}
      role="menu"
      ...
      className={`fixed z-[140] w-[184px] origin-top-right rounded-md border border-neutral-200 bg-panel p-1.5 shadow-[0_16px_40px_-12px_rgba(24,27,32,0.35)] transition-[opacity,transform] duration-[var(--motion-base)] ease-standard motion-reduce:transition-none motion-reduce:transform-none ${
        state === "open" ? "scale-100 opacity-100" : "scale-[0.97] opacity-0"
      }`}
    >
      <Button variant="text" type="button" role="menuitem" className={itemClass} onClick={onPrimary}>
        {primaryLabel}
      </Button>
    </div>,
    document.body,
