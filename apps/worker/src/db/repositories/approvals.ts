 ...
 * pending one in the same statement so the partial unique index
...
export async function createApprovalRequest(
  ...
  const result = await db.execute(sql`
    ...
    -- but a workflow_runs lock conflict now fails the approval statement too.
      ...
      where ${workflowRuns.ticketKey} = ${input.ticketKey}
        and ${workflowRuns.status} = 'awaiting'
        and ${workflowRuns.runId} <> ${input.runId}
      returning ${workflowRuns.runId}
    ), inserted as (
