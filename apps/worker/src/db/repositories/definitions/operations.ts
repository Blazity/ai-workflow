...
} from "../../../workflow-definition/layout.js";
...
export async function createWorkflowDefinition(
  ...
  let initialVersion: number | null;
  try {
    const inserted = await createDefinitionsRepository(db).createWithInitialVersion({
      name: input.name,
      layout,
      layoutRevision: input.seed ? 1 : 0,
      actorId: input.actor.id,
      actorLabel: input.actor.label,
      initialDefinition: semantic,
    });
    initialVersion = inserted.initialVersion;
    const rows = await db
      .select()
      .from(workflowDefinitions)
  ...
  const current =
    initialVersion === null
      ? null
      : await getWorkflowDefinitionVersion(db, created.id, initialVersion);
  if (initialVersion !== null && !current) {
