...
import { webhookTriggerEndpoints } from "../schema.js";
...
export function mintConnectedWebhookEndpointsForDefinition(
  encryptionKey: string,
  input: Parameters<typeof mintWebhookEndpointsForDefinition>[2],
) {
  return mintWebhookEndpointsForDefinition(getDb(), encryptionKey, input);
}

export function getConnectedWebhookEndpointById(endpointId: string) {
  return getWebhookEndpointById(getDb(), endpointId);
}

export function readConnectedWebhookEndpointForDelivery(endpointId: string) {
  return readWebhookEndpointForDelivery(getDb(), endpointId);
}

export function getConnectedWebhookEndpointForNode(definitionId: number, nodeId: string) {
  return getWebhookEndpointForNode(getDb(), definitionId, nodeId);
}

export function rotateConnectedWebhookEndpointSecret(
  encryptionKey: string,
  endpointId: string,
  options?: { force?: boolean },
) {
  return rotateWebhookEndpointSecret(getDb(), encryptionKey, endpointId, options);
}

export function setConnectedWebhookEndpointSecret(
  encryptionKey: string,
  endpointId: string,
  secret: string,
) {
  return setWebhookEndpointSecret(getDb(), encryptionKey, endpointId, secret);
}

export function revealConnectedWebhookEndpointSecret(encryptionKey: string, endpointId: string) {
  return revealWebhookEndpointSecret(getDb(), encryptionKey, endpointId);
}

export function revokeConnectedWebhookEndpoint(endpointId: string) {
  return revokeWebhookEndpoint(getDb(), endpointId);
}

export function unrevokeConnectedWebhookEndpoint(
  encryptionKey: string,
  endpointId: string,
) {
  return unrevokeWebhookEndpoint(getDb(), encryptionKey, endpointId);
}
