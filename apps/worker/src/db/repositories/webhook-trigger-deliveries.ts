...
import type { JsonValue, WebhookDeliveryOutcome } from "@shared/contracts";
...
interface WebhookTriggerEntry {
  ...
  priority: string;
  payload: JsonValue;
  supportCase?: WebhookSupportCase;
}
...
export function acceptConnectedWebhookDelivery(accepted: AcceptedWebhookDelivery) {
  return acceptWebhookDelivery(getDb(), accepted);
}

export function getConnectedWebhookDelivery(endpointId: string, deliveryId: string) {
  return getWebhookDelivery(getDb(), endpointId, deliveryId);
}

export function coalesceConnectedPendingWebhookDelivery(accepted: AcceptedWebhookDelivery) {
  return coalescePendingWebhookDelivery(getDb(), accepted);
}

export function drainConnectedOldestPendingWebhookDelivery(subjectKey: string) {
  return drainOldestPendingWebhookDelivery(getDb(), subjectKey);
}

export function listConnectedPendingWebhookDeliveries(limit: number) {
  return listPendingWebhookDeliveries(getDb(), limit);
}

export function completeConnectedWebhookDelivery(
  endpointId: string,
  deliveryId: string,
  result: StoredWebhookResult,
) {
  return completeWebhookDelivery(getDb(), endpointId, deliveryId, result);
}

export function recordConnectedWebhookDeliveryStarted(
  identity: WebhookDeliveryIdentity,
  ownerToken: string,
  runId: string,
) {
  return recordWebhookDeliveryStarted(getDb(), identity, ownerToken, runId);
}

export function recordConnectedStartedWebhookTriggerDelivery(
  input: { endpointId: string; deliveryId: string; subjectKey: string; ownerToken: string; runId: string },
) {
  return recordWebhookDeliveryStarted(
    getDb(),
    { endpointId: input.endpointId, deliveryId: input.deliveryId, subjectKey: input.subjectKey },
    input.ownerToken,
    input.runId,
  );
}

export function listConnectedRecentWebhookDeliveries(endpointId: string, limit: number) {
  return listRecentWebhookDeliveries(getDb(), endpointId, limit);
}

export function sweepConnectedWebhookDeliveries(now: Date = new Date()) {
  return sweepWebhookDeliveries(getDb(), now);
}

export async function consumeWebhookTriggerRate(
  db: Db,
  input: { endpointId: string; windowStart: Date; kind: "ingress" | "inbox" },
): Promise<number> {
  const { webhookTriggerRateLimits } = await import("../schema.js");
  const rows = await db.insert(webhookTriggerRateLimits).values({ ...input, count: 1 })
    .onConflictDoUpdate({
      target: [webhookTriggerRateLimits.endpointId, webhookTriggerRateLimits.windowStart, webhookTriggerRateLimits.kind],
      set: { count: sql`${webhookTriggerRateLimits.count} + 1` },
    }).returning({ count: webhookTriggerRateLimits.count });
  return rows[0]?.count ?? 1;
}

export function consumeConnectedWebhookTriggerRate(
  input: Parameters<typeof consumeWebhookTriggerRate>[1],
): Promise<number> {
  return consumeWebhookTriggerRate(getDb(), input);
}

export async function sweepExpiredWebhookTriggerRates(db: Db, cutoff: Date): Promise<void> {
  const { webhookTriggerRateLimits } = await import("../schema.js");
  await db.delete(webhookTriggerRateLimits).where(lt(webhookTriggerRateLimits.windowStart, cutoff));
}

export function sweepConnectedExpiredWebhookTriggerRates(cutoff: Date): Promise<void> {
  return sweepExpiredWebhookTriggerRates(getDb(), cutoff);
}

export async function incrementWebhookTriggerRejection(
  db: Db,
  input: { endpointId: string; windowStart: Date; reason: string },
): Promise<void> {
  const { webhookTriggerRejectionCounters } = await import("../schema.js");
  await db.insert(webhookTriggerRejectionCounters).values({ ...input, count: 1 })
    .onConflictDoUpdate({
      target: [webhookTriggerRejectionCounters.endpointId, webhookTriggerRejectionCounters.windowStart, webhookTriggerRejectionCounters.reason],
      set: { count: sql`${webhookTriggerRejectionCounters.count} + 1` },
    });
}

export function incrementConnectedWebhookTriggerRejection(
  input: Parameters<typeof incrementWebhookTriggerRejection>[1],
): Promise<void> {
  return incrementWebhookTriggerRejection(getDb(), input);
}

export async function listWebhookTriggerRejections(
  db: Db,
  input: { endpointId: string; windowStart: Date },
) {
  const { webhookTriggerRejectionCounters } = await import("../schema.js");
  return db.select({ reason: webhookTriggerRejectionCounters.reason, count: webhookTriggerRejectionCounters.count })
    .from(webhookTriggerRejectionCounters)
    .where(and(eq(webhookTriggerRejectionCounters.endpointId, input.endpointId), eq(webhookTriggerRejectionCounters.windowStart, input.windowStart)))
    .orderBy(desc(webhookTriggerRejectionCounters.count));
}

export function listConnectedWebhookTriggerRejections(
  input: Parameters<typeof listWebhookTriggerRejections>[1],
) {
  return listWebhookTriggerRejections(getDb(), input);
}

export async function sweepExpiredWebhookTriggerRejections(db: Db, cutoff: Date): Promise<void> {
  const { webhookTriggerRejectionCounters } = await import("../schema.js");
  await db.delete(webhookTriggerRejectionCounters).where(lt(webhookTriggerRejectionCounters.windowStart, cutoff));
}

export function sweepConnectedExpiredWebhookTriggerRejections(cutoff: Date): Promise<void> {
  return sweepExpiredWebhookTriggerRejections(getDb(), cutoff);
}
