...
async function definitionDeploymentValidation(
  ...
  issues: ReturnType<typeof validateWorkflowDefinitionIssuesForDeployment>;
}> {
  const parsed = structural(definition);
  const resolvedHarnessProfiles =
...
export async function restoreWorkflowDefinitionVersion(db: Db, input: { definitionId: number; version: number; actor: WorkflowDefinitionActor }) {
  ...
  return appendWorkflowDefinitionVersionWithPolicy(db, {
    definitionId: input.definitionId,
    definition: await validStored(await blockContractsOn(db), requireRunnableVersion(source)),
...
async function connectedHarnessProfilesForDefinition(
  definition: WorkflowDefinition,
): Promise<ResolvedHarnessProfilesForDeployment> {
  let organizationId: Promise<string> | null = null;
  return resolveHarnessProfilesForDefinition(
    definition,
    async ({ profileId, version }) => {
      organizationId ??= connectedDashboardOrganizationId();
      return (
        await resolveConnectedVerifiedHarnessProfileVersion({
...
async function connectedDashboardOrganizationId(): Promise<string> {
  const {
  ...
  const organizationSettings = dashboardOrganizationSettings(
    await loadSettingsSnapshot(),
  );
  const organization =
    await createConnectedAuthRepository().findOrganizationBySlug(
      organizationSettings.slug,
    );
  if (!organization) {
    throw new Error(
      `Dashboard organization "${organizationSettings.slug}" is unavailable.`,
    );
  }
