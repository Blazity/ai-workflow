...
export async function saveRepositoryProfile(input: {
  ...
  assertVcsProviderAvailable(input.request.provider, "save");
  const rulesError =
...
export async function setRepositoryCatalogEnabled(input: {
  ...
  if (input.enabled) assertVcsProviderAvailable(existing.provider, "enable");
...
export async function activateRepositoryCatalog(input: {
  ...
  for (const entry of entries) {
    if (entry.enabled) assertVcsProviderAvailable(entry.provider, "activate");
  }
