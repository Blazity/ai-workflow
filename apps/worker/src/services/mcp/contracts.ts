/**
 * The MCP contract vocabulary: scopes, published tool names, error codes, the
 * actor a verified token resolves to, and the shapes the audit and idempotency
 * ledgers store.
 *
 * It lives below the transport because the services that write those ledgers and
 * resolve that actor are the ones that own these types; `mcp/contracts.ts`
...
export type McpAuditInput = {
  ...
  latencyMs: number;
  occurredAt: Date;
  /** The published contract this attempt was served under. Supplied by the
   *  transport, because the hash is computed over the tool catalog it publishes. */
  contractHash: string;
};
