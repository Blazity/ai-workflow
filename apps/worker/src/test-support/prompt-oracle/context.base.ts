 ...
 *
 * The prompt assemblers of apps/worker/src/sandbox/context.ts exactly as they
 * were at 57dc151bbadadffebafceec7768f669b1dd6d0ed, before the runtime prompt
 * was split into named parts. The oracle tests hold the current assemblers to
 * these bytes over a generated input matrix, so a change in what a model reads
 * shows up as a failing comparison rather than as nobody noticing.
 *
 * The helpers the assemblers import (formatAttachmentsIndex and formatBytes,
 * the workspace path helpers, the review ledger work-item predicate) are
 * copied below as they were at the same commit, so a change inside one of them
 * cannot move both sides of the comparison at once. The edits to the copied
 * text, none of which changes a byte it renders: em dashes and the warning
 * emoji are written as escapes, which produce the same strings, and the
 * formatters and one type the tests never import are not exported.
 */
import type {
  PRComment,
  CheckRunResult,
...
// --- Copied from sandbox/attachments.ts at the same commit.
...
function formatAttachmentsIndex(
...
// --- Copied from sandbox/repo-workspace.ts at the same commit.

const WORKSPACE_ROOT_DIR = "/vercel/sandbox";
const WORKSPACE_REPOS_DIR = "/vercel/sandbox/repos";

function buildRepoSlug(repoPath: string): string {
  return repoPath
    .trim()
    .toLowerCase()
    .split("/")
...
function buildProviderRepoSlug(provider: SelectedRepository["provider"], repoPath: string): string {
  return `${provider}__${buildRepoSlug(repoPath)}`;
}

function buildWorkspaceLocalPath(
...
function isValidWorkspaceLocalPath(repository: {
  provider: SelectedRepository["provider"];
  repoPath: string;
  localPath: string;
}): boolean {
...
// --- Copied from adapters/vcs/vcs-bot-identity.ts at the same commit.

function isReviewLedgerWorkItem(thread: {
...
// --- sandbox/context.ts at the same commit, unchanged below this line.
...
interface SelectedRepositoryPromptContext {
...
function formatCheckResults(checks: CheckRunResult[]): string {
