...
import {
  ...
  parseSkillLocator,
  refreshRepositorySkillArtifact,
...
class FakeRepository implements RepositorySkillSource {
  defaultBranch = "main";
  ...
  trees = new Map<
    string,
    { entries: RepositorySkillTreeEntry[]; truncated: boolean }
  >();
...
describe("GitHub skill source parsing", () => {
  it.each([
    ...
    expect(parseSkillLocator(source)).toEqual(expected);
  });
  ...
  it.each([
    ...
    expect(() => parseSkillLocator(source)).toThrow(
      HarnessSkillImportError,
    );
  });
});
...
describe("GitHub skill discovery", () => {
  ...
  it("fails before snapshot download when discovery exceeds the candidate cap", async () => {
    ...
    await expect(
      discoverRepositorySkills({ repository, source: "acme/skills" }),
  ...
  it("rejects truncated or inaccessible repository data without exposing provider errors", async () => {
    ...
    await expect(
      discoverRepositorySkills({ repository: truncated, source: "acme/skills" }),
  ...
  it("keeps a client status the skill source declared, and refuses to repeat its outage", async () => {
    // The source lives in an integration and cannot throw core's error class,
    // so it declares the status on the error instead. Flatten that and "this
    // snapshot is too big" becomes indistinguishable from "we could not read
    // the repository", which is a different thing for the person to fix.
    const declared = new FakeRepository();
    addSkill(declared);
    declared.getFiles = () =>
      Promise.reject(
        Object.assign(new Error("Repository snapshot is too large"), {
          status: 413,
        }),
      );
    await expect(
      discoverRepositorySkills({ repository: declared, source: "acme/skills" }),
    ).rejects.toMatchObject({
      statusCode: 413,
      message: "Repository snapshot is too large",
    });

    // A provider describing its own outage is not a client error, and its
    // message is not ours to put on the dashboard.
    const outage = new FakeRepository();
    addSkill(outage);
    outage.getFiles = () =>
      Promise.reject(
        Object.assign(new Error("upstream 503 from api.example.com"), {
          status: 503,
        }),
      );
    await expect(
      discoverRepositorySkills({ repository: outage, source: "acme/skills" }),
    ).rejects.toMatchObject({
      statusCode: 422,
      message:
        "GitHub repository could not be read with the organization installation",
...
describe("GitHub skill import", () => {
  ...
  it("imports and reuses an exact-SHA artifact with hashes, modes, and no public bytes", async () => {
    ...
    const [artifact] = await importRepositorySkills(db, {
      repository,
    ...
    const [again] = await importRepositorySkills(db, {
      repository,
      organizationId: "org-skills",
      actorId: "admin",
      request,
  ...
  it("imports a 41-skill repository snapshot in one bounded bulk operation", async () => {
    ...
    const artifacts = await importRepositorySkills(db, {
      repository,
      organizationId: "org-skills",
      actorId: "admin",
      request: {
  ...
  it("refreshes from the repository default branch into a new immutable artifact", async () => {
    const repository = new FakeRepository();
    addSkill(repository);
    const [original] = await importRepositorySkills(db, {
      repository,
      organizationId: "org-skills",
      actorId: "admin",
      request: {
    ...
    const refreshed = await refreshRepositorySkillArtifact(db, {
      repository,
      organizationId: "org-skills",
      actorId: "admin",
  ...
  it("refuses a deployment-local artifact before touching the provider", async () => {
    ...
    const error = await refreshRepositorySkillArtifact(db, {
      repository,
      organizationId: "org-skills",
      actorId: "admin",
      artifactHash,
