...
export function inviteEmailTemplate(input: InviteEmailTemplateInput): EmailTemplate {
  ...
  return {
    ...
    html: renderEmailHtml({
      ...
      body: `${inviter} invited you to join ${input.organizationName}.`,
...
function renderEmailHtml(input: {
  ...
  const actionHref = escapeHtml(input.actionHref);
...
<html>
  <body style="font-family: Arial, sans-serif; color: #111827; line-height: 1.5;">
    ...
    <p style="margin: 0 0 20px;">
      <a href="${actionHref}" style="display: inline-block; background: #111827; color: #ffffff; padding: 10px 14px; text-decoration: none; border-radius: 6px;">${actionLabel}</a>
    </p>
    <p style="margin: 0; color: #4b5563; font-size: 14px;">If the button does not work, paste this link into your browser:</p>
    <p style="margin: 8px 0 0; color: #4b5563; font-size: 14px;">${fallbackUrl}</p>
  </body>
</html>`;
}
