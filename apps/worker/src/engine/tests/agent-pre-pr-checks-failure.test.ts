...
import { repositoryScriptFailureEntry, reconcileRunBudgetErrorAtBoundary, repositoryScriptsOutput, repositoryScriptsStatus, shouldRethrowAgentExecutionError, unhandledAgentExecutionError } from "../agent-workflow.js";
import { PRE_PR_CHECKS_FAILURE_CAUSE_MAX_LENGTH, PRE_PR_CHECKS_FAILURE_STACK_TAIL_MAX_LENGTH, definitionRequestsRepairCycles, isRepositoryScriptsFailurePhase, prePrChecksFailureInput, prePrChecksFailureMustPropagate, prePrChecksFailureReport, failureExitPhase, nodeCanRecordGate, recoverLatestRepositoryScriptsFailureFromSteps, repositoryScriptsFailureComment } from "../helpers/repository-failure.js";
import { prePrChecksFailureMessage } from "../steps/repository-failure.js";
import { describePrePrChecksFailureStep } from "../steps/repository-failure.js";
import {
  expectOutputConformsToRegistry,
  makeCtx,
  runControlErrorCases as blockRunControlErrorCases,
} from "../blocks/support/test-support.js";
import type {
  WorkflowBlockType,
...
describe("repository scripts block output", () => {
  ...
  it("keeps the gate output recoverable by finalize and run_scripts out of it", async () => {
    const { recoverPrePrGateFromSteps } = await import("../blocks/finalize-workspace/execute.js");
    const shared = repositoryScriptsOutput(
      engineResult({
        results: ranOneCommand(),
        groupStatuses: [groupStatus("checks", "passed")],
