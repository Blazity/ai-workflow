...
// The text a `send_message` block hands to the messaging capability. A v2 run
// handles the authored message in this order:
...
//   2. resolveMessageInput (same file) picks a bound `message` input over the
//      configured text and trims it.
// What the connected provider then makes of it is the provider's: the Slack
// renderer is held by integrations/slack/format.test.ts.
// Step 2 runs in executeBlock's "send_message" case, which is not
...
/** Steps 1 and 2 above: the text the block hands the capability, or why it fails. */
function resolveMessage(
  ...
  return { message: resolveMessageInput(resolution.configuration, {}) };
}

describe("send_message: the text the block sends", () => {
  ...
  it("fails the block instead of posting a legacy {{ticket_key}} as literal braces", () => {
    expect(resolveMessage("Heads up: {{ticket_key}} is ready")).toEqual({
      failed: "send_message message contains an unresolved placeholder.",
    });
  });
