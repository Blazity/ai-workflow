/**
 * The environment-backed allowlist, still live inside a run.
 *
 * These cases came verbatim from `services/dispatch/repo-allowlist.test.ts`,
 * which used to re-export this module and now answers from the repository
 * catalog instead. The module below did not change and is still what guards
 * repository discovery and branch or PR creation inside a run, so its
 * assertions moved here rather than disappearing with that rewrite. The engine
 * wave (stage X) gives a run its own enabled list and deletes the module; this
 * file goes with it.
 */
