...
describe("finalize_workspace paramsSchema", () => {
  it("accepts empty params and rejects retired authoring params", () => {
    ...
    expect(manifest.paramsSchema.safeParse({ legacyRequiredChecks: ["checks.with dots"] }).success).toBe(false);
    expect(manifest.paramsSchema.safeParse({ requiredChecks: ["checks-1"] }).success).toBe(false);
