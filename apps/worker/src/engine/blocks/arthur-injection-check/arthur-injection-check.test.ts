...
describe("arthur_injection_check paramsSchema", () => {
  it("accepts empty params and rejects the retired contentFromStep param", () => {
    ...
    expect(manifest.paramsSchema.safeParse({ contentFromStep: "step-1" }).success).toBe(false);
    expect(manifest.paramsSchema.safeParse({ legacyContentFromStep: "step-1" }).success).toBe(false);
