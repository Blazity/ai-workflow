...
describe("leak_review paramsSchema", () => {
  it("accepts the authored params and rejects unknown or out-of-range values", () => {
    expect(leakManifest.paramsSchema.safeParse({}).success).toBe(true);
    expect(
      leakManifest.paramsSchema.safeParse({ model: "claude-haiku-4-5", llmScan: false, maxDiffBytes: 4096 })
        .success,
    ).toBe(true);
    expect(leakManifest.paramsSchema.safeParse({ llmScan: "false" }).success).toBe(false);
    expect(leakManifest.paramsSchema.safeParse({ maxDiffBytes: 0 }).success).toBe(false);
    expect(leakManifest.paramsSchema.safeParse({ maxDiffBytes: 262_145 }).success).toBe(false);
    expect(leakManifest.paramsSchema.safeParse({ extra: 1 }).success).toBe(false);
  });
});
