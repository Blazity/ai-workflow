...
describe("human_question paramsSchema", () => {
  it("accepts empty params, a questions array, and rejects unknown keys", () => {
    ...
    expect(manifest.paramsSchema.safeParse({ questions: ["Q1"] }).success).toBe(true);
    expect(manifest.paramsSchema.safeParse({ questions: [""] }).success).toBe(false);
  ...
  it("accepts an optional suggestedAnswers array of non-empty strings", () => {
    expect(manifest.paramsSchema.safeParse({ suggestedAnswers: ["Yes", "No"] }).success).toBe(true);
    expect(manifest.paramsSchema.safeParse({ suggestedAnswers: [""] }).success).toBe(false);
  });
});
