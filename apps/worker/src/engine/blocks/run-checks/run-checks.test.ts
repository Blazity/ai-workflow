...
const runChecksParamsSchema = BLOCK_PARAM_SCHEMAS.run_checks;
const runChecksDiagnostics = [
  {
    input: { commands: ["pnpm lint"], skipReason: "Do not run" },
    issue: {
      path: ["skipReason"],
  ...
  {
    input: { groups: ["test"], skipReason: "Not applicable." },
    issue: {
      path: ["skipReason"],
  ...
  {
    input: { groups: ["test"], commands: ["pnpm lint"] },
    issue: {
      path: ["groups"],
...
function expectRunChecksDiagnostic(input: unknown, issue: unknown) {
  const result = runChecksParamsSchema.safeParse(input);
  expect(result.success).toBe(false);
  if (result.success) return;
  expect(result.error.issues.map(({ path, message }) => ({ path, message }))).toEqual([issue]);
}

describe("run_checks paramsSchema", () => {
  it("accepts empty params, commands, or a required skip reason", () => {
    expect(runChecksParamsSchema.safeParse({}).success).toBe(true);
    expect(runChecksParamsSchema.safeParse({ commands: ["pnpm lint"] }).success).toBe(true);
    expect(runChecksParamsSchema.safeParse({ skipReason: "Not applicable to docs-only work." }).success)
      .toBe(true);
    expect(runChecksParamsSchema.safeParse({ commands: [""] }).success).toBe(false);
    expect(runChecksParamsSchema.safeParse({ skipReason: " " }).success).toBe(false);
    expect(
      runChecksParamsSchema.safeParse({
        commands: ["pnpm lint"],
        skipReason: "Do not run",
      }).success,
    ).toBe(false);
    expect(runChecksParamsSchema.safeParse({ extra: 1 }).success).toBe(false);
  });

  it("reports exact generated-schema diagnostics for mutually exclusive modes", () => {
    expect(runChecksParamsSchema.safeParse({ groups: ["test"] }).success).toBe(true);
    expect(runChecksParamsSchema.safeParse({ groups: ["test", "lint"] }).success).toBe(true);
    ...
    expect(runChecksParamsSchema.safeParse({ groups: ["Test"] }).success).toBe(false);
    expect(runChecksParamsSchema.safeParse({ groups: [] }).success).toBe(false);
    for (const diagnostic of runChecksDiagnostics) {
      expectRunChecksDiagnostic(diagnostic.input, diagnostic.issue);
    }
  });
});
