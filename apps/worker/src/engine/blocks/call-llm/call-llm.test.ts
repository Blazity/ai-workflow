...
import { execute, resolveCallLlmTarget } from "./execute.js";
...
describe("call_llm paramsSchema", () => {
  it("allows a binding-only prompt, leaves the model unset by default, and rejects unknown keys", () => {
    const parsed = manifest.paramsSchema.safeParse({ prompt: "hi" });
    ...
    expect(manifest.paramsSchema.safeParse({ prompt: "hi", extra: 1 }).success).toBe(false);
  });

  it("accepts a claude/codex provider and rejects any other value", () => {
    expect(manifest.paramsSchema.safeParse({ prompt: "hi", provider: "codex" }).success).toBe(true);
    expect(manifest.paramsSchema.safeParse({ prompt: "hi", provider: "claude" }).success).toBe(true);
    expect(manifest.paramsSchema.safeParse({ prompt: "hi", provider: "gemini" }).success).toBe(false);
  });
});
