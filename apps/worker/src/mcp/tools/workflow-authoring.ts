 ...
 * be able to fire. The rule lives in trigger-liveness.ts, shared with
 * workflows.list's `armed`.
 */
function dormantTriggerNodes(
  services: McpToolDependencies["services"],
  definitionId: number,
  graph: WorkflowDefinition,
  enabled: boolean,
): Promise<string[]> {
  return dormantTriggerNodeIds(services, definitionId, triggerNodesOf(graph), enabled);
}
