...
export const MCP_TOOL_CATALOG = {
  ...
  "settings.list": {
    description:
      "Every setting this deployment has, resolved: the value in force, where it came from (`stored` row, `environment` variable, or registry `default`), the group the Settings page files it under, the description an operator reads, and `appliesToRunsInFlight`, which is \"immediate\" for a key only a request, a cron tick or this transport reads and \"next run\" for every key a workflow body reads, because a run carries its settings from its start so a replay sees what the first execution saw. `editable` says whether settings.set may write the key at all and `role` who may. Two groups are refused on this surface: `repositories`, because activating the catalog is repositories.activate, which states what stops passing first, and `mcp`, because those keys configure this transport itself and an agent must not be able to raise its own limits or switch its own access off mid-session; both are changed on the dashboard Settings page. `requiresRedeploy` marks a key the running code reads from the environment rather than from the store, so the variable is its only answer: this surface refuses to write it, a stored row left over from an earlier release loses to the environment, and the value changes only once the variable changes and the worker is redeployed, which is why those keys report `appliesToRunsInFlight: \"after redeploy\"` instead of \"next run\". `migratedVariablesSet` names the environment variables this deployment still sets whose values the store has taken over: those are what an operator deletes from the deployment, redeploying afterwards, and the list is empty when the cleanup release can ship. `migratedVariablesUnstored` is the subset nothing has stored yet, read back from the table rather than assumed from an import having run: a name there must NOT be removed, because the value lives nowhere else. A `requiresRedeploy` key is on neither list, because the running code reads its variable directly. No credential is in this list by construction: keys, tokens, database and auth URLs stay in the environment and are deliberately absent from the registry, and any secret this deployment does hold is redacted from every reply on this surface anyway.",
    inputSchema: z.object({}).strict().default({}),
