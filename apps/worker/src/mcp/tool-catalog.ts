...
export const MCP_TOOL_CATALOG = {
  ...
  "settings.reset": {
    description:
      "Remove a setting's stored row so its registry default answers again. This is not a second write: it hands the key back to the resolution order, and the reply names the resulting `value` and `source`. Owner only -- stricter than the HTTP route, which admits an admin -- and only on a token with a person behind it, because the value that takes over is not one the caller stated. The same `mcp` group settings.set refuses is refused here: clearing a transport key is a change to what the platform may do, by the back door. `reason` is required and is recorded as the version row for this key, so the history shows the clearing alongside the writes. A key with nothing stored comes back as `removed: false` with the value that was already resolving, which is a success and not an error. Idempotent per idempotencyKey.",
    inputSchema: z
      .object({
        key: z.string().trim().min(1).max(SETTING_KEY_MAX_LENGTH),
        reason: z.string().trim().min(1).max(SETTING_REASON_MAX_LENGTH),
