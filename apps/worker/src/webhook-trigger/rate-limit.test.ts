...
describe("webhook rate limiting", () => {
  ...
  it("counts every request in a window and refuses past the limit", async () => {
    ...
    for (let request = 0; request < 4; request += 1) {
      decisions.push(await checkAndIncrementWebhookRate(db, "wh_a", 2, inWindow));
    }
  ...
  it("starts a fresh count in the next minute and never shares one across endpoints", async () => {
    await checkAndIncrementWebhookRate(db, "wh_a", 1, inWindow);
    await expect(
      checkAndIncrementWebhookRate(db, "wh_a", 1, inWindow),
    ).resolves.toMatchObject({ count: 2, allowed: false });

    await expect(
      checkAndIncrementWebhookRate(db, "wh_a", 1, nextWindow),
    ).resolves.toMatchObject({ count: 1, allowed: true });
    await expect(
      checkAndIncrementWebhookRate(db, "wh_b", 1, inWindow),
    ).resolves.toMatchObject({ count: 1, allowed: true });
  });

  it("defaults to the documented per-minute limit", async () => {
    await expect(
      checkAndIncrementWebhookRate(db, "wh_a", undefined, inWindow),
    ).resolves.toMatchObject({ limit: DEFAULT_WEBHOOK_RATE_LIMIT_PER_MINUTE });
    expect(DEFAULT_WEBHOOK_RATE_LIMIT_PER_MINUTE).toBe(60);
  });

  it("sweeps windows older than an hour and keeps the live ones", async () => {
    await checkAndIncrementWebhookRate(db, "wh_a", 60, new Date("2026-02-01T08:00:00.000Z"));
    await checkAndIncrementWebhookRate(db, "wh_a", 60, new Date("2026-02-01T09:30:00.000Z"));
    await checkAndIncrementWebhookRate(db, "wh_a", 60, inWindow);
