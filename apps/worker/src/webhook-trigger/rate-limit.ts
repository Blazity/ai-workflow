...
/** Requests one endpoint may accept per minute before it starts refusing. High
 *  enough that a normal integration never notices, low enough that a looping
 *  sender cannot fill the delivery inbox. */
...
export async function checkAndIncrementWebhookRate(
  db: Db,
  endpointId: string,
  limitPerMinute: number = DEFAULT_WEBHOOK_RATE_LIMIT_PER_MINUTE,
  now: Date = new Date(),
): Promise<WebhookRateDecision> {
  const windowStart = webhookRateWindowStart(now);
  const rows = await db
    .insert(webhookTriggerRateLimits)
    .values({ endpointId, windowStart, count: 1 })
    .onConflictDoUpdate({
      target: [webhookTriggerRateLimits.endpointId, webhookTriggerRateLimits.windowStart],
