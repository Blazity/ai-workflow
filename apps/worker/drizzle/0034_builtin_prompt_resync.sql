...
WITH "new_body" AS (
  SELECT $aiw_prompt_resync_0034$# Instructions
...
The file may contain a `Human decisions` section (between `<!-- human-decisions:start -->` and `<!-- human-decisions:end -->`) that is maintained automatically. Preserve it exactly: do not edit or remove it.$aiw_prompt_resync_0034$::text AS "body"
), "corrected" AS (
...
- Do NOT describe sandbox mechanics, tooling you had to work around, or actions you were forbidden to take. If something genuinely blocked the ticket, that belongs in `error` with `result: "failed"`, not in `summary`.$aiw_prompt_resync_0034$::text AS "body"
), "corrected" AS (
  UPDATE "prompt_library_versions" AS "v"
  SET "body" = (SELECT "body" FROM "new_body")
  WHERE "v"."version" = 1
...
- Do NOT describe sandbox mechanics or actions you were forbidden to take. If review itself could not be completed, that belongs in `error` with `result: "failed"`, not in `feedback`.$aiw_prompt_resync_0034$::text AS "body"
), "corrected" AS (
  UPDATE "prompt_library_versions" AS "v"
  SET "body" = (SELECT "body" FROM "new_body")
  WHERE "v"."version" = 1
