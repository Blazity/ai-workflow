...
-- ON CONFLICT keeps the deploy robust: if pre-migration data already has two
-- enabled definitions sharing a trigger type (the old precheck was racy), the
-- backfill keeps the first binding instead of aborting the migration.
