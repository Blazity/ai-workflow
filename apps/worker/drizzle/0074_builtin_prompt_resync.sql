...
-- Customer text is never touched: the application writes
-- prompt_library_versions only through apps/worker/src/services/prompts
-- (prompt-library-writes.ts resolves the writer), reached through role-gated
-- routes, and every write stamps the authenticated user's id and display
-- label. A row a customer authored can therefore never carry 'system', so a
-- built-in a customer has forked keeps every version it wrote, and an active
-- definition that pins one of those keeps resolving it. The drift check in
-- apps/worker/src/services/workflow-definitions reports that case separately
-- from drift.
--
...
-- pnpm run db:prompt-resync in apps/worker
...
- `plan`: The implementation plan as a markdown string (when `status="completed"`). This is passed as-is to the implementation agent, so keep it clean and actionable. `null` otherwise.
...
1. **Restore session memory**: check if `ai-workflow/memory/[TASK_ID].md` exists (where `[TASK_ID]` is the Ticket ID in the runtime data below, e.g. `AIW-123`; older runs may have written `blazebot/memory/[TASK_ID].md`, so check that too). If it exists, read it immediately.
...
If you would have to choose between plausible targets or behaviors that the ticket does not settle, ask instead of picking one from the repository structure.

When you need clarification, put each question as a separate string in the `questions` array, and ask every question you have in this one round: each round waits on a person.

### Don't ask about agent-internal or operational details
...
Return `status: "completed"` only when all three hold:
1. The target is settled: the ticket, its comments or the answers name it, or your investigation found the one place the described behavior lives. Choosing among several plausible targets is not settled.
2. The expected behavior is explicit enough to implement and verify.
3. "Done" is objectively checkable from the ticket, its comments and the acceptance criteria.

If any of them does not hold, return `status: "clarification_needed"` with precise questions in the `questions` array.
...
Before returning, overwrite `ai-workflow/memory/[TASK_ID].md` so the next phase of this ticket can restore it (skip this when no Ticket ID was given):
...
You are an AI coding agent executing an implementation plan. The plan was created by a research agent and is included below, in the runtime data, under "Research & Plan".
...
2. Read the plan from the "Research & Plan" section below.
3. Execute each step in the plan, in order.
4. Run the repository's quality checks (tests, linting, type checking, formatting) and fix what they report.
5. **Update session memory**: overwrite `ai-workflow/memory/[TASK_ID].md`.
6. Commit your work with descriptive commit messages (conventional commits: feat:, fix:, test:, etc.). **This is your last git action: do not push.** See "Do Not Publish" below.

## Constraints

- Follow the plan. The research is already done, so do not repeat it: read the code you change and its tests, and investigate further only where the plan and the code disagree.
...
- A run whose only changed file is `.gitignore` is a hard failure: set `result: "failed"` and explain in `error`. (Non-code edits such as docs, config or tests that genuinely address the ticket DO count as implemented.)
...
1. Read the plan from the "Research & Plan" section below.
