...
- Define product-behaviour settings once in `packages/contracts/settings-registry.ts`; ordinary values live in the settings store, and owner or admin changes through `apps/worker/src/routes/api/v1/settings.patch.ts` record the actor and reason in settings history.
