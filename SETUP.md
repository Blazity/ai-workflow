...
8. [Register the VCS webhook](#8-register-the-vcs-webhook)
...
ai-workflow uses Neon Postgres as its run registry and as its store for
workflow definitions, approvals and telemetry (atomic claim/release for
concurrent runs, dedupe, locking). Tables are created automatically:
migrations run during every deploy's build step.
...
`AGENT_ALLOWED_REPOS` is the hard guard on which repositories the agent may ever touch inside a run: it filters repository discovery and hard-guards branch/PR creation, so an off-list repo fails the run with `Refusing to branch <repo>: not in AGENT_ALLOWED_REPOS` rather than being silently skipped.

**It no longer decides dispatch.** Since the catalog consumers stage, whether a pull request or merge request event, the legacy post-PR gate behind them, a manual pull request dispatch or an MCP dispatch may act on a repository is answered by the **repository catalog** (Repositories in the cockpit sidebar), not by this variable: a refusal now reads "this repository is not enabled in the repository catalog". A workflow definition's repository pin is a selection inside the catalog and no longer extends dispatch. While the catalog is **not activated** every accessible repository still counts as enabled, so a deployment that has never opened that page behaves exactly as it did. A ticket-driven run is not on that list: it still chooses its repositories inside the run, and the engine stage moves that choice too.

**Warning: keep this variable in step with the catalog.** What is left for it is the seed below and the engine reads listed after it, which the engine stage removes. Until then the in-run guard described above still applies, so enabling a repository in the catalog that this variable omits starts runs that fail LATE, inside the engine at promotion or pull request creation, after an agent invocation has been spent. Enabling such a row in the dashboard answers with that warning; heed it and add the repository here as well.
...
Unlike most variables here, `AGENT_ALLOWED_REPOS` is read from `process.env` directly (in `apps/worker/src/infra/repository-allowlist-env.ts`, for `apps/worker/src/engine/support/repo-allowlist.ts`) rather than through the validated `env.ts` singleton, so it is deliberately absent from `env.ts` and a malformed value will not crash startup.

Every deploy imports this variable into the repository catalog: the build-time seed (`apps/worker/scripts/db-seed-repository-catalog.ts`) creates one enabled catalog row per entry, plus one for every repository a stored workflow definition pins, and activates the catalog exactly when the variable is non-empty, recording the activation as `seeded from AGENT_ALLOWED_REPOS` so it can be reviewed later. An entry whose VCS provider cannot be determined (no definition pins it, no catalog row names it, and no VCS provider is configured) fails the build rather than being guessed at. The variable is now read by that seed and by the engine's in-run guard only; the engine stage moves the in-run guard onto the run's own enabled list, and the cleanup stage removes the variable entirely, after which a value set here fails environment validation with a message naming the Repositories page that replaced it.
...
2. **Confirm the App has the right permissions and event subscription:**
   ...
   - Subscribe to events → **Pull request**, **Check run**, **Pull request review** (all checked). See [`docs/GITHUB-APP-SETUP.md` §5](./docs/runbooks/GITHUB-APP-SETUP.md#5-subscribe-to-events). Without **Check run** and **Pull request review**, the `trigger_pr_checks_failed` and `trigger_pr_review` workflow triggers never fire.
...
- Every current document → [docs/index.md](./docs/index.md)
- Architecture and workflow internals → [README.md](./README.md) and [docs/architecture/workflow-definition.md](./docs/architecture/workflow-definition.md)
- Spec → [docs/product/SPEC.md](./docs/product/SPEC.md)
- User stories → [docs/product/user-stories.md](./docs/product/user-stories.md)
