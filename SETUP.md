...
| `GITHUB_BOT_LOGIN`, `GITLAB_BOT_LOGIN`        | unset (the bot's own reviews are not filtered)                                                                                                              | Provider-specific login of the bot's own VCS account. Reviews authored by it are ignored so the bot cannot trigger a run off its own review (`trigger_pr_review`). For a GitHub App this is usually `<app-slug>[bot]`. These values take precedence over `VCS_BOT_LOGIN`. |
| `VCS_BOT_LOGIN`                               | unset                                                                                                                                                        | Legacy shared fallback used when the matching provider-specific bot login is unset. |
...
For GitLab.com, configure the project webhook instead: see [`docs/GITLAB-SETUP.md`](./docs/GITLAB-SETUP.md). The webhook URL is `https://<your-vercel-domain>/webhooks/gitlab`, the **Secret token** field must match `GITLAB_WEBHOOK_SECRET`, and **Merge request events**, **Pipeline events** (the **Pipeline Hook**), and **Comments** (the **Note Hook**) are required. Pipeline events drive `trigger_pr_checks_failed`; Comments drive `trigger_pr_review`. GitLab does not emit a Note Hook for a request-changes action without a review summary, so that summaryless case cannot trigger the workflow. Do not use GitLab's newer **Signing token** flow until the worker implements signing-token verification.
