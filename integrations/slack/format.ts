/**
 * A ticket event as Slack mrkdwn.
 *
 * Moved from `apps/worker/src/adapters/messaging/format.ts`. What changed: the
 * link comes from core (`MessagingTicket.url`) instead of being built from a
 * tracker base url this package has no business knowing, and the failure line
 ...
 *
 * Slack-native `<url|label>` syntax, sent as the `text` of a message, which is
 * what Slack renders as mrkdwn.
 */
export function formatTicketEvent(event: TicketEvent, ticket: MessagingTicket): string {
  const link = ticketLink(ticket);
  const emoji = EVENT_EMOJI[event.kind];
  ...
  switch (event.kind) {
    ...
    case "pr_ready": {
      ...
      // extraText is user/ticket-derived (a Send message block's message
...
/** The ticket as a link when core gave one, and as its bare key when it did not. */
function ticketLink(ticket: MessagingTicket): string {
