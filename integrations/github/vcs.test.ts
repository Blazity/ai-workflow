...
vi.mock("./auth", () => ({
  ...
  getBotIdentity: vi.fn(async () => ({
    ...
    email: "7+ai-workflow[bot]@users.noreply.github.com",
  })),
}));
...
describe("GitHubAdapter", () => {
  ...
  describe("the operational surfaces core detects by method", () => {
    it("maps every repository the installation can see", async () => {
      mockOctokit.paginate.mockResolvedValueOnce([
        ...
        // No owner object and no default branch: GitHub omits both on a
        // repository the installation can see but not read in full, and a
        // listing that threw there would hide every other repository too.
        {
          full_name: "test-org/legacy",
          name: "legacy",
          html_url: "https://github.com/test-org/legacy",
        },
      ]);

      await expect(ghAdapter().listRepositories()).resolves.toEqual([
        {
          provider: "github",
        ...
        {
          provider: "github",
          repoPath: "test-org/legacy",
          name: "legacy",
          owner: "test-org",
          defaultBranch: "",
          description: "",
          webUrl: "https://github.com/test-org/legacy",
          topics: [],
          archived: false,
          private: false,
        },
    ...
    it("hands a sandbox the App's own token and commit identity", async () => {
      // x-access-token is the only user name an installation token authenticates
      // as over HTTPS; the identity is what makes the commit render as the App.
      await expect(ghAdapter().sandboxCredentials()).resolves.toEqual({
  ...
  describe("review ledger", () => {
    ...
    it("does not call a parked thread reopened while our reply is the last note", async () => {
      ...
      // Both arguments, or the matcher would pass on arity alone and never see
      // the event it is here to refuse.
