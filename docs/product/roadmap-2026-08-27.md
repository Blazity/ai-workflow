...
Delivered 2026-09-19 to 2026-09-20 by
[the agent visibility plan](../plans/2026-09-19-agent-visibility.md): every
model send a run makes is recorded as an Agent Briefing, section by section,
each section cut into named parts carrying an origin, and read back in the
dashboard and through MCP as the same bytes. That closes "show the exact
effective instructions and context used by every run" for what goes into a
send. It does not close the inventory bullet (the harness CLI's own system
prompt and tool definitions are out of scope) and it does not start the
ownership, approval and rollback bullet, which is a separate project.
...
Delivered 2026-09-20 by the same plan: a repository the catalog relates to one
the work names becomes a candidate the run takes without asking, checked out
read only, recorded under its own `related_repository` origin and re-derived on
the next run, which closes "allow one repository event to trigger a workflow
that can access approved related repositories". Configuring the visible scope
per trigger is the trigger repository policy of
[the work scope plan](../plans/2026-09-15-repository-work-scope.md).
...
Partly delivered 2026-09-20 by the same plan: a block attempt's Briefing tab
names, per send, the model id, the digest of the output schema, the pinned
skills delivered, the repositories with the map the agent really received, and
every context source, which is most of "show each block's inputs, outputs,
model, tools, skills, repositories, context sources, and evidence"; the harness
CLI's own tool definitions are out of scope. "Show only context explicitly
bound to an agent" and "prevent access to unbound runtime data" are not done:
naming every part of a prompt makes them possible, and nothing enforces either
yet.
