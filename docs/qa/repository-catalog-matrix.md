...
| R20 | The five run-start variables render, and nothing else does | rules using all five plus `{{ticket_description}}` | run | `ticket_key, ticket_url, branch_name, repo_path, repo_default_branch` render; `{{ticket_description}}` is left standing braces and all for a legacy stored row, and is logged ONCE per compiled prompt as `repository_rules_unresolved_variable` | The prompt; the log line and its count | `apps/worker/src/engine/steps/repository-rules.test.ts`; the whole set rendered in one artefact at `apps/worker/src/engine/steps/repository-rules.matrix.test.ts` | low |
