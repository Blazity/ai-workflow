...
| E7 (L25/D2) | Disable every enabled catalog row so the enabled set is empty. | On `f693c6f58ec80dfc9a89b079e281e3cb2d9171b9` at 17:12:47Z to 17:13:02Z, six `repositories.set_enabled` calls (rows 4, 1, 3, 6, 5, 2) answered `enabledRemaining` 5 down to 0; the Repositories page then showed "No repository is enabled. Every dispatch is refused until one is enabled again." with every row NOT ENABLED. Rows re-enabled at 17:15:08Z to 17:15:16Z, `enabledRemaining` back to 6. | PASS | |
| E8 (R04/D8) | Dispatch a ticket while no repository is enabled. | AWP-172 dispatched manually at 17:13:53Z on definition 14 version 10 during the E7 window: run `wrun_01M2DW5PFF4XN076W60BPD8904` failed at start with `repositoryAccess` null (no sandbox), and the ticket was bounced to Backlog at 17:14:15Z with the comment "No repository is enabled in the catalog. Enable one on the Repositories page and move the ticket again." | PASS | |
...
| R1 (relationships) | Supply a related repository to the run prompt and verify that the agent carries the relationship into its merge request description. | AWP-173 run `wrun_01M2E1YFYCHM7JP0AW0YK2T67G` succeeded in 772 s. Repository 2 was GitLab `filipmaszota3/ai-workflow-integration-test`, profile version 11, with relationship `{repositoryId: 6, kind: "mirror_of", note: "QA R1 probe"}`. The agent quoted the "Related repositories" sentence from its prompt verbatim in MR !51; commit `e8c05788`. | PASS | |
...
| R06 | Ticket names an unknown repository | activated catalog | dispatch a ticket naming `acme/nope` | Clarification: "Research requested {provider}:{repoPath}, which is not available to this run. To use it, enable it on the Repositories page and start a new run. This run can only add another repository alongside the ones already attached." closing with "Reply "none" to continue without {provider}:{repoPath}; the run stops if the agent cannot plan without it." (`unavailableRepositoryClarification` in `engine/repository-discovery/runner.ts`) | Clarification text on the run | `apps/worker/src/services/repository-discovery/runner.matrix.test.ts:62` (catalog miss), `:87` (present but not usable) | med |
...
| R20 | Rule variables render on production | profile rule with all five variables `{{ticket_key}} {{ticket_url}} {{branch_name}} {{repo_path}} {{repo_default_branch}}` on gitlab `filipmaszota3/ai-workflow-integration-test`, catalog description with two paragraphs | transition AWP-174 to Ai | the MR head commit ends with the trailer carrying the five values; an unknown token is refused on write with the allowed list | prod 9c1b539761b129f3b65ff7324c00e2116e8e94d2, run wrun_01M2EE9AWAQEZA0YDJ4R24KCPR, MR !52 head 68c7d7f8fd9e1f718096a251c1a6fe209ad6ba88, trailer `Trailer: AWP-174 | https://blazity.atlassian.net/browse/AWP-174 | ai-workflow/awp-174 | filipmaszota3/ai-workflow-integration-test | main`; `repositories.upsert` with `{{unknown_token}}` returned VALIDATION_FAILED | `apps/worker/src/engine/steps/repository-rules.test.ts`, golden `repository-rules-prompt.golden.txt` | low |
| R21 | Repository variables differ per repository | one rules text, two repositories | run touching both | Each compilation resolves `repo_path` and `repo_default_branch` from its own catalog row, falling back to the run's provider branch only when the catalog branch is empty | Both sections in the prompt | `repository-rules.test.ts`; `prompt-vars.test.ts` | low |
...
| id | scenario | preconditions | steps | expected | evidence to capture | automated test | risk |
|---|---|---|---|---|---|---|---|
| T01 | Activation has no settings back door | any | inspect `settings.list`; query an unknown key; activate through the Repositories page | No activation entry exists in the settings registry; unknown keys return 400; only the catalog activation route changes catalog state | Settings list, 400 body, catalog state | contracts registry tests; `apps/worker/src/routes/api/v1/settings.test.ts`; repository-catalog route tests | low |
| T02 | Catalog state answers activation | an admin activated the catalog | UI: Settings page | The setup overview reads `catalogState` from the catalog list route and says who activated it and when (`apps/dashboard/AGENTS.md` Settings section) | Screenshot showing "Activated" | `apps/dashboard/lib/settings/overview.test.ts`; Settings screen tests | med |
| T03 | No repository settings group | any | UI: Settings | Repository access and activation live on the Repositories page; Settings has no repository group or dead controls | Settings screenshot | dashboard settings tests | low |
...
| T06 | `requiresRedeploy` keys | `DASHBOARD_ORG_SLUG`, `MCP_ALLOW_PUBLIC_DCR`, `PRE_PR_CHECKS_ALLOWED_ENV` | inspect Settings and `settings.list`; try a settings write | Listed with `requiresRedeploy: true` and the environment value in force; rendered in the read-only Deployment variables block; both HTTP and MCP writes are refused | The list, screen, and refusal | registry tests; settings route/tool tests; Settings screen tests | med |
...
| M05 | `repositories.activate` admits an owner or an admin, person-backed (owner only until 2026-09-23) | admin person token; then member person token; then owner client-credentials token | `repositories.activate` | The admin activates; the member and the client-credentials token are refused, as the dashboard refuses them | The success plus two tool errors | `repositories.test.ts:1228`, `:1250`, `:1285` | low |
| M06 | `settings.reset` admits an owner or an admin, the same rule as `POST /api/v1/settings/reset` (owner only until 2026-09-23) | admin person token; then member person token | `settings.reset` | The admin clears the stored value; the member is refused and nothing is removed; the HTTP route answers both actors the same way | The success, the tool error, both HTTP answers | `settings.test.ts:626`, `:655`, `:777`; `routes/api/v1/settings.test.ts:465`, `:483` | low |
...
| U12 | Variable palette shows exactly five | entry, Rules tab | open the palette | `ticket_key, ticket_url, branch_name, repo_path, repo_default_branch` and nothing else, so the menu, highlight and renderer agree | Screenshot of the palette | `apps/dashboard/app/(cockpit)/repositories/repository-entry.matrix.test.tsx` | med |
| U13 | Unknown variable in Rules | type `{{ticket_description}}` | entry, Rules tab | Inline Field error names the token and all five allowed variables; Save stays disabled. HTTP and MCP return the same validation message. A legacy stored row still renders the token literally with one runtime warning | Screenshot; both refusals; compiled prompt | `repository-entry.validation.test.tsx`; `repositories.test.ts`; `authoring.test.ts`; `repository-rules.test.ts` | low |
...
- U12 the variable palette offering exactly the five names, asserted against `REPOSITORY_RULES_VARIABLES` so the menu cannot drift from the renderer.
...
- A golden fixture of the compiled prompt containing a `Repository rules for <owner/name>` section with all five variables rendered, so a change to `effective-prompt.ts` or the variable set fails visibly rather than silently changing what the agent reads.
