...
- The remote MCP surface is integrated on source `main`. The committed/local target remains 22 tools, 11 public error codes, and contract hash `881de2fae17a183a44645d8d6c6c8c8089e604fcc2197d98d4f27acb66ec2f7b`.
- The canonical MCP endpoint, [https://ai-workflow-app-eight.vercel.app](https://ai-workflow-app-eight.vercel.app), points to deployment `dpl_6vS3JVz5zNQUCezQMkbYpexQEeQo` at commit `bbdcc5ac`. `GET /mcp` returns 405, unauthenticated initialize returns 401 with `mcp:read`, and protected-resource metadata returns 200. Authenticated production `tools/list`, `system.capabilities`, and exact-hash verification remain pending.
- OAuth DCR, consent, PKCE exchange, token verification, and authenticated MCP initialization have been proven against source production. Final post-merge contract dogfood is in progress.
- Signed-in dashboard sessions now bridge to MCP consent through a one-minute, single-use, hashed handoff token; raw session tokens stay server-side. AIW-273 is deployed on source production, but interactive signed-in dashboard-session OAuth handoff remains pending because the Chrome session is unavailable.
...
| --- | --- | --- |
| [AIW-239](https://blazity.atlassian.net/browse/AIW-239) — remote MCP endpoint | Merged / final production verification | [PR #256](https://github.com/Blazity/ai-workflow/pull/256), [PR #263](https://github.com/Blazity/ai-workflow/pull/263), [PR #267](https://github.com/Blazity/ai-workflow/pull/267), and [PR #271](https://github.com/Blazity/ai-workflow/pull/271); the production deployment is ready and the final authenticated contract sweep is running |
| [AIW-270](https://blazity.atlassian.net/browse/AIW-270) — MCP OAuth consent | Merged / final production verification | [PR #262](https://github.com/Blazity/ai-workflow/pull/262), [PR #264](https://github.com/Blazity/ai-workflow/pull/264), [PR #266](https://github.com/Blazity/ai-workflow/pull/266), and [PR #268](https://github.com/Blazity/ai-workflow/pull/268); fresh production DCR, consent, PKCE exchange, and authenticated initialize succeeded |
| [AIW-273](https://blazity.atlassian.net/browse/AIW-273) — reuse the signed-in dashboard session for MCP authorization | Merged / source deployed; interactive verification pending | [PR #273](https://github.com/Blazity/ai-workflow/pull/273); the secure session handoff is deployed on source production, but the signed-in dashboard-session OAuth handoff still needs interactive verification because the Chrome session is unavailable |
...
| [AIW-276](https://blazity.atlassian.net/browse/AIW-276) — production MCP bundle omitted `zod/v3` | Merged / production verified | [PR #274](https://github.com/Blazity/ai-workflow/pull/274); the zod/v3 Vercel pre-auth crash was fixed and canonical `/mcp` reaches the auth layer. The committed/local target remains 22 tools, 11 errors, and hash `881de2fae17a183a44645d8d6c6c8c8089e604fcc2197d98d4f27acb66ec2f7b`; authenticated production `tools/list`, `system.capabilities`, and exact-hash verification remain pending |
| [AIW-271](https://blazity.atlassian.net/browse/AIW-271) — permanent RUNNING and phantom starts | Merged / cleanup verification | [PR #269](https://github.com/Blazity/ai-workflow/pull/269); lost-claim settlement and PR/webhook start-row checks are deployed, while the reported production rows still require explicit verification and recorded cleanup |
...
AIW-273, AIW-274, and AIW-276 are integrated into source `main`. AIW-274 implementation is done, while its real live Jira/DB finalization race remains part of stress verification. The remaining work is to finish the authenticated MCP contract sweep, verify the AIW-271 production rows, settle the AIW-275 product rule, reconcile Jira statuses, and record the stress-test evidence. Focused local tests, typechecks, generated-artifact checks, production build checks, and combined-main smoke tests are the source acceptance evidence; CI is not the acceptance gate.
