...
- The remote MCP surface is integrated on source `main`: 22 versioned tools, five OAuth scopes, 11 public error codes, authoring, run control, and ticket writes are represented by one committed contract. The target contract hash is `881de2fae17a183a44645d8d6c6c8c8089e604fcc2197d98d4f27acb66ec2f7b`.
- The canonical MCP endpoint, [https://ai-workflow-app-eight.vercel.app](https://ai-workflow-app-eight.vercel.app), points to deployment `dpl_6vS3JVz5zNQUCezQMkbYpexQEeQo` at commit `bbdcc5ac`. `GET /mcp` returns 405, unauthenticated initialize returns 401 with `mcp:read`, and protected-resource metadata returns 200. Authenticated `tools/list` and `system.capabilities` remain pending.
...
| [AIW-276](https://blazity.atlassian.net/browse/AIW-276) — production MCP bundle omitted `zod/v3` | Merged / production verified | [PR #274](https://github.com/Blazity/ai-workflow/pull/274); the zod/v3 Vercel pre-auth crash was fixed, the 22-tool contract hash is unchanged, and canonical `/mcp` reaches the auth layer |
