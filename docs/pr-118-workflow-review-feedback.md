...
4. **Remediation event safety — earlier review position.** Authenticate provider events, allowlist eligible producers, filter workflow-authored events, deduplicate deliveries, require an actionable terminal failure for the current head, and ignore passing, cancelled, neutral, skipped, superseded, stale-head, and otherwise non-actionable events.
