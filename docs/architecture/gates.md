...
The table below is every gate script that can turn a pull request red, and the
one place they are described; the root `gates` script in `package.json` is the
...
| Lint | Oxlint diagnostics for every app, `scripts/`, `packages/` and `integrations/` | `scripts/gates/lint.mjs` | Any diagnostic is reported |
...
| Package contracts | Descriptions for every package under `packages/` and `integrations/` (and `integrations/_fixtures/` when present) | `scripts/gates/package-contracts.mjs` | A package has no non-empty description |
