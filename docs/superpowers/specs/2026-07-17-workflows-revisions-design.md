...
Webhook authentication remains the first gate. Accepted deliveries receive a durable provider delivery record before dispatch. Delivery identity is unique per provider, so redelivery is idempotent.

Event normalization includes provider, producer, repository, PR/MR number, head SHA, and delivery ID. Defaults admit GitHub Actions/GitLab CI failures and human-authored review feedback while excluding AI Workflow's own checks/comments. Passing, cancelled, neutral, skipped, superseded, non-terminal, stale-head, and otherwise non-actionable results start nothing. The provider API re-reads the current PR/MR head both before immediate dispatch and before a queued delivery starts.
...
The merged event carries a real ticket only when correlation succeeds. Before Update Ticket Status calls the provider, it records a short-lived transition intent containing the ticket, owning run, and destination. The Jira webhook consumes a matching intent without cancelling that run; unmatched human moves keep the existing cancellation behavior. The active claim remains until all downstream notification blocks finish.
