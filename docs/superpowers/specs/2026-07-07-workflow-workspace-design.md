...
7. **V5 · Move ticket after merge** — `PR/MR merged` configured with `scope: workflow_owned` → Update ticket status → optional Slack message. Workflow-owned scope statically guarantees a real ticket. A short-lived transition intent prevents the Jira webhook echo from cancelling its own still-running workflow.
