...
| 11 | Ratchet to zero (implemented 2026-09-11): remaining boundary violations, knip findings and `db/client` fence count driven to zero using the same production-only, exact-module-specifier definition as stage 7; all baseline files deleted; rules are hard; create `docs/architecture/gates.md` with the complete ladder and no baseline column. Terminates because every directory has a tier (ADR-001) | Import boundaries, DB repository | `apps/worker/src/**`, `scripts/gates/*.baseline.json`, `docs/architecture/gates.md`; after 10 | sonnet | no | no | yes (sweep) | all `*.baseline.json` deleted; `pnpm run verify:changed` and CI green with hard rules; `docs/architecture/gates.md` contains the complete ladder with no baseline column |
