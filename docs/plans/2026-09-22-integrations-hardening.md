...
| F56 | major | CONFIRMED | `integrations/slack/manifest.ts:56` | The slash-command allowlist (`SLACK_ALLOWED_USER_IDS`) is modelled as a Slack connection field. | B-connections | OPEN: round G (Slack operator settings, webhook-only use), brief lanes/fix-groups/brief-slack-operator-settings.md |
