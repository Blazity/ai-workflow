...
- **Settings gets an Integrations screen.** One card per integration the build
...
the gate reviews the addition, and ADR-009 records it in a change log section.
...
- Screen layout, card design, copy, empty, loading, error and permission
  states, and whether Integrations is a tab inside Settings or its own page
  under it, within `DESIGN.md`.
...
|---|---|---|---|---|---|---|---|---|---|
| S0 | The contract exists: an integration can be described in types, and the capability ports live in the SDK | SDK contract | `integrations/sdk/**`, `docs/adr/ADR-009-integrations.md`, `CONTEXT.md`, `pnpm-workspace.yaml`, re-export shims in `apps/worker/src/adapters/*/types.ts` | opus | tight | yes | conformance helper only | yes (inventory of what the five providers use from the worker) | `pnpm run typecheck` green; the inventory is attached to ADR-009 and every item maps to a context member or a stated reason it stays in core; a fixture integration type-checks against the manifest, the context (including `llm`) and each port; ADR-009 states decisions 1 to 20 with reasons; `pnpm run verify:changed` green |
