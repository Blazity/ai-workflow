...
| [plans/2026-09-23-memory-package.md](./plans/2026-09-23-memory-package.md) | Draft: agent memory as one worker module, Mem0 with scored recall and updates, a memory ledger, store switching, and the stage table |
| [plans/2026-09-23-memory-package-ux.md](./plans/2026-09-23-memory-package-ux.md) | Draft (revision 2): the UX spec for agent memory: the run card per agent invocation (in the prompt, looked up, added on touch, never looked at), `/memory` Needs review inbox, folders with trust, status, pins and human fixes, the "why didn't it remember X" verdicts, forget, copy and store switches, MCP answer shapes, one vocabulary, the data contract for stages 7, 7b and 8, and what stage 9 must prove |
| [plans/2026-09-23-memory-quality-and-routing.md](./plans/2026-09-23-memory-quality-and-routing.md) | Draft: memory quality, folders and on-demand retrieval on top of the memory plan: topic and area taxonomy with paths resolved against the listing, pull-first retrieval (small core and index in the prompt, a read-only tree and an offline command in the sandbox, a just-in-time hook that injects an area once when the agent touches it), per-invocation lookup and feedback collection, learning that waits for the PR to merge, the Needs review inbox, stages 4b, 4c, 6c, 6d, 6e and 7b |
