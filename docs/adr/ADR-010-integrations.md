...
| 2026-09-23 | review fixes (round G) | `IntegrationWebhookManifest.requires` and the `WebhookConnectionValues` it types; conformance code `webhook_requires_invalid` | The route served a webhook only when the whole integration was usable, so Slack's slash command needed the bot token and the channel, which it never uses: the answer goes to `response_url`. On main it needed only `SLACK_SIGNING_SECRET`. Which fields a webhook reads is the integration's to declare and core's to enforce, for any integration: with `requires`, core serves the webhook while the integration is enabled and those fields have values in the active source, and the webhook's `ctx.connection` holds exactly those. Without it the whole connection is needed, which is right for a webhook that goes on to call its provider. Additive. |
