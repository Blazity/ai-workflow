...
| `SLACK_SIGNING_SECRET`; HMAC check with `node:crypto` | `integration-settings.ts:87`, `services/slack/verify.ts:1,31-39` | secret connection field; own webhook handler (S9 slot). The webhook requires it and nothing else (`webhook.requires`, 2026-09-23), as on main, where the command needed only this variable |
| `SLACK_ALLOWED_USER_IDS` | `integration-settings.ts:95-97` | a stored setting in core's settings system: who may use a slash command is operator behaviour. S9 made it a connection field, which put it in the run's pin and in one source only; since 2026-09-23 the Slack manifest declares it as an operator setting (`settings`), core stores and serves it with every other setting under this same name, the variable is read while nothing is stored, and the webhook reads it as `ctx.settings.allowedUserIds` |
...
line each as they move a provider out. It reached nothing in the hardening
round: the function and the parameter are gone, and core serves none of the
three (plan 2026-09-22-integrations-hardening, H1.3).
...
- Integration settings that cannot be read mean "nothing usable" where the
  ...
  `usableIntegrations` is the "nothing usable" reading of it. (Deleted in the
  hardening round, plan H1.2: every caller reads `resolveUsableIntegrations`
  and says what unreadable settings mean where it is.)
...
| 2026-09-23 | memory list by subject | `MemoryStoreAdapter.list` takes an optional `subjectKey` (the port's own word, from `MemoryStoredDocumentRef`); the built-in store applies it in its query, `GET /api/v1/memory?subjectKey=` and `memory.list` pass it, and the repository page lists its repository through it, the key built by `repoSubjectKey` in `@shared/contracts` | The repository page read `facts` and `lessons` by name, which a provider choosing its own `docPath`s breaks. Reading from the unfiltered listing instead fixed that and broke something else: the listing is the newest page of every subject (100 by default), so on a busy deployment an older repository showed no documents. Core holds a subject-filtered listing to its subject for every provider: a listing carrying another subject's document is cut to the asked one and reported incomplete (`memory_list_subject_filter_ignored`), so a provider that ignores the filter costs a person the notice, not a false "nothing stored". Additive for providers (an optional field), a behaviour change for the route: `subjectKey` alone used to be a 400. |
| 2026-09-23 | review fixes (round G) | `IntegrationManifest.settings` with the type `IntegrationSetting` (a `string-list` with a `default` and an optional `env` read while nothing is stored); `integrationSettingKey`, `integrationSettingDefinition`, `integrationSettingDefinitionsOf`, `settingDefinitionsOf` and `integrationSettingValues` (`settings.ts`); `IntegrationWebhookContext`, what `webhook.receive` and `deliver` now receive, which adds `settings`; conformance code `setting_invalid`. In `@integrations/registry`, `settingDefinitions`, `settingDefinition` and `integrationSettingDefinitions`. In `@shared/contracts`, `IntegrationDto.movedToSettings` (`IntegrationMovedValueDto`), the validation reason `list_entry_invalid` with `SETTING_LIST_ENTRY_RULE`, the settings group `integrations`, an ordinary key's `environmentVariable` read between a stored row and the default, and a lookup argument on `validateSettingsPatch` and `resolveSettingWithoutStoredRow`. Slack's connection field `allowedUserIds` removed | Decision 9 says operator behaviour is never a connection value, and Slack's slash-command allowlist was one: it entered the pin, so adding a colleague stopped every run posting to Slack with `reconfigured`, and it lived in one source, so switching Slack to stored values dropped it and opened the command to the workspace. An integration now declares the settings it reads, and core stores, validates, versions and serves them with its own (Settings page, MCP `settings.*`, history) from one joined list, under a key derived from the integration id (`SLACK_ALLOWED_USER_IDS`, the variable main read). Read when used and never pinned: only a webhook receives them, loaded with the request's settings, and core answers 503 without calling it when they cannot be read, because an unreadable allowlist is not an empty one. A run does not receive them: its settings are frozen at run start, and carrying these in that snapshot is a change to a step that needs a drain, left to the first integration whose block needs one. A stored connection version that still carries `allowedUserIds` is read without it (no reader of a connection knows the key any more). Main has no stored connections, but preview and demo deployments share production's database, so a preview of this branch may have written one there; with no setting row and no variable the setting's default then lets the whole workspace run the command. That is not silent: the card names the value and the setting that applies instead (`IntegrationDto.movedToSettings`, derived per read by `storedValuesMovedToSettings`), each webhook read that would have used it logs `integration_stored_value_not_read`, and the next save drops it, since a version is built from the connection fields alone. Setting list entries are refused on write when blank or holding a comma (`list_entry_invalid`, one sentence, `SETTING_LIST_ENTRY_RULE`, for the HTTP API, MCP and the dashboard); a variable is still split on commas as main split it. Additive for a manifest; a test that builds a webhook context by hand adds `settings`. The connection shape moves for Slack; production pins no Slack connection before this branch merges, so the merge drain already covers it. |
| 2026-09-23 | review fixes (round G) | `IntegrationWebhookManifest.requires` and the `WebhookConnectionValues` it types, and `label` beside it; conformance code `webhook_requires_invalid`; `IntegrationDto.webhook` (`IntegrationWebhookDto`: `label`, `requires`, `served`) in `@shared/contracts` | The route served a webhook only when the whole integration was usable, so Slack's slash command needed the bot token and the channel, which it never uses: the answer goes to `response_url`. On main it needed only `SLACK_SIGNING_SECRET`. Which fields a webhook reads is the integration's to declare and core's to enforce, for any integration: with `requires`, core serves the webhook while the integration is enabled and those fields have values in the active source, and the webhook's `ctx.connection` holds exactly those. Without it the whole connection is needed, which is right for a webhook that goes on to call its provider. The card has to say so, or a deployment that registered only the command reads Failing and nothing else: `served` is derived on each read, never stored, by the one read the route makes (`readWebhookConnection`), and the dashboard words it only where it differs from the rest of the integration; `label` is what it calls the webhook. Additive. |
| 2026-09-23 | review fixes (round G) | `RunControlOutcome`'s failure carries `reference` instead of `message`; `RunPullRequest.noun`, stamped by core with `reference` from `changeRequestNaming`, and `pullRequestNoun` | Core put its raw error in `message`, and Slack posted it to the channel, where a failed query quotes its SQL and parameters. The answers were already values and never sentences except this one: core now logs the error under a diagnostic id (`AIW-DIAG-run-control-...`) and hands the integration that id, and the integration writes the sentence, shown only to the person who typed the command. Not additive: one producer (the webhook route) and one consumer (Slack's renderer), both in this repository, changed together. The noun completes the change request naming above: Slack said `PR ready (!12)` for a merge request. Additive. |
| 2026-09-23 | review fixes | The optional `ticketUrl(key)` on `IssueTrackerAdapter`; `capabilityLabel(id)` beside `INTEGRATION_CAPABILITIES` | Core spelled every tracker's links itself, as `<Site URL>/browse/<KEY>` from the raw connection value behind a Jira-shaped key filter, so a Site URL with a path linked nowhere and a second tracker would have been linked as Jira. The tracker now says what page a key opens, pure and never throwing, null for a key that is not one of its tickets; core records it on the run and asks it through one function (plan H1.1). `capabilityLabel` is the one reader of the labels the table already carried, so the editor and the dashboard cannot name a capability two ways. Both additive: a tracker without `ticketUrl` gives no links rather than wrong ones. |
| 2026-09-23 | vcs consolidation | `vcs-extensions.ts` holds the optional VCS surfaces (gate status, rich gate status, changed files, review publication, manual dispatch) with their guards, and `VcsIntegrationAdapter` declares each as an optional member; `IntegrationCapabilityFactories.vcs` returns `VcsIntegrationAdapter`. `review-markers.ts` holds every pull request marker family with its readers (`isOurOwnVcsComment`, the ledger, the review round's summary, head, finding and legacy markers). `VCS_BOT_LOGIN_FIELD`, `VCS_LEGACY_BOT_LOGIN_FIELD`, `normalizeVcsLogin` and `vcsLoginsMatch`, with conformance code `vcs_bot_login_missing`. `providerAnswer`, `ProviderAnswer` and `PROVIDER_VERDICT_HEADERS` replace `providerAnswerOf`, and `readProviderFailure` reads an answer a client kept on `cause.response`. `getPRHeadSha` left the port: nothing called it, and its two implementations failed differently from `getPRHead` | The extension types, the marker grammar and the login field each lived in core and again in each provider package, and the copies had begun to differ; a marker already posted must keep reading, so its literals are pinned in `review-markers.test.ts`. Core's deferred adapter answered a capability question for every member name, so a provider without `listPRFiles` read as capable until the call failed: it now forwards the port only, and core asks the adapter it resolved. A GitHub rate limit visible only in headers read as a refused credential once core's redacted copy dropped the headers; the copy now keeps `status` and the verdict headers. GitHub and GitLab now send every request through `ctx.http` (Octokit's `request.fetch`, a Gitbeaker requester), so they get core's deadline, retry policy, lifetime and redaction. Not additive in three places: a `vcs` manifest without a `botLogin` field fails conformance (GitHub and GitLab declare one), `providerAnswerOf` is gone (no caller outside the SDK), and a block that called `getPRHeadSha` must read `getPRHead(...).headSha` (none on this branch; a provider that still implements it keeps compiling). |
