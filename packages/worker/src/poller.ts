...
async function checkMissedWebhooks(): Promise<void> {
  const projectKey = workerEnv.JIRA_PROJECT_KEY;

  if (!workerEnv.JIRA_BASE_URL || !workerEnv.JIRA_USER_EMAIL || !workerEnv.JIRA_API_TOKEN) {
...
async function checkStuckJobs(
  ...
  const thresholdMs = workerEnv.STUCK_JOB_THRESHOLD_MS ?? workerEnv.JOB_TIMEOUT_MS * 2;
