...
vi.mock("@blazebot/shared", () => ({
  ...
  tickets: { externalId: "externalId", source: "source", id: "id" },
  runAttempts: { id: "id", ticketId: "ticketId", containerId: "containerId" },
  ticketQueue: { add: vi.fn(), getJob: vi.fn() },
  ...
  createTicketLogger: vi.fn((..._args: unknown[]) => mockChildLogger),
  createRunLogger: vi.fn((..._args: unknown[]) => mockChildLogger),
  JiraClient: vi.fn().mockImplementation(function JiraClientMock(this: unknown) {
    return mockJira;
  }),
  GitHubClient: vi.fn().mockImplementation(function GitHubClientMock(this: unknown) {
    return mockGitHub;
  }),
...
describe("worker handler", () => {
  ...
  describe("review_fix handler", () => {
    it("fetches ticket, PR comments, conflict status, runs sandbox, and moves to AI Review on success", async () => {
      ...
      const { db } = await import("@blazebot/shared");
