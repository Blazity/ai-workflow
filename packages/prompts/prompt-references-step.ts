...
import { VARIABLE_PARAM_KEYS } from "./prompt-vars";
import {
    ...
    for (const key of keys) {
      ...
      if (typeof value === "string") {
        ...
        for (const entry of resolved.manifest) {
          ...
          nodeManifest.set(`${entry.promptId}@${entry.requestedVersion}`, entry);
        }
        nodeSlots = coalescePromptSlotDefinitions([...nodeSlots, ...resolved.slots]);
